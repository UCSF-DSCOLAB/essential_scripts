# NOTE - this is Arjun's code -- not my own
# this alternate dotplot function uses the mean to saturate the plots 
# which helps with visualizing ADT data

suppressPackageStartupMessages({
  if (!'package:Seurat' %in% search()) library(Seurat)
  if (!'package:rlang' %in% search()) library(rlang)
  if (!'package:cowplot' %in% search()) library(cowplot)
})

FuncPctDotPlot <- function (object, assay = NULL, features, cols = c("lightgrey", 
                                                                     "blue"), col.min = -2.5, col.max = 2.5, 
                            dot.min = 0, dot.scale = 6, 
                            idents = NULL, group.by = NULL, split.by = NULL, cluster.idents = FALSE, 
                            scale = TRUE, scale.by = "radius", scale.min = NA, scale.max = NA, saturate.func=mean, ...) 
{
  ...
  plot <- ggplot(data = data.plot, mapping = aes_string(x = "features.plot", 
                                                        y = "id")) + geom_point(mapping = aes_string(size = "pct.exp", 
                                                                                                     color = color.by)) + 
    scale.func(range = c(0, dot.scale), limits = c(scale.min, scale.max)) + theme(axis.title.x = element_blank(),                                                                                                                                                                axis.title.y = element_blank()) + guides(size = guide_legend(title = "Percent Expressed")) + 
