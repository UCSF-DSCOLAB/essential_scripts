...
#' @param min_pct: Genes with counts per million (CPM) more than 1 in at least \code{min_pct} % of samples of the smaller group are retained. Default 60%
#' @return \code{counts} after removing lowly expressed genes
remove_low_expression <- function(counts, metadata, dge_by, min_pct=60) {
  ...
  feats = names(which(rowSums(cpm(counts) > 1) > (min_gsize * min_pct/100)))
