...
run_mast <- function(
    ...
    ### ToDo: remove mast.freq.expressed.min by implementing in other internal function
    mast.freq.expressed.min <- 0.2
    ...
    sca <- SceToSingleCellAssay(sce, check_sanity = FALSE) # Unfortunately, the 'sanity check' can incorrectly call logged data not logged.
    ...
    ts_log("\twith model formula: ", model)
